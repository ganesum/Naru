...
{
    /// <summary>
    /// Provides a TaskScheduler that uses an I/O completion port for concurrency control.
    /// </summary>
    public sealed class IOCompletionPortTaskScheduler : TaskScheduler, IDisposable
    {
        /// <summary>
        /// The queue of tasks to be scheduled.
        /// </summary>
        private readonly ConcurrentQueue<Task> _tasks;

        /// <summary>
        /// The I/O completion port to use for concurrency control.
        /// </summary>
        private readonly IOCompletionPort _iocp;

        /// <summary>
        /// Whether the current thread is a scheduler thread.
        /// </summary>
        private readonly ThreadLocal<bool> _schedulerThread;

        /// <summary>
        /// Event used to wait for all threads to shutdown.
        /// </summary>
        ...
        /// <summary>
        /// Gets a list of all tasks scheduled to this scheduler.
        /// </summary>
        /// <returns>
        /// An enumerable of all scheduled tasks.
        /// </returns>
        ...
        /// <summary>
        /// 
        /// Queues a task to this scheduler for execution.</summary>
        ...
        /// <summary>
        /// 
        /// Provides a simple managed wrapper for an I/O completion port.</summary>
        private sealed class IOCompletionPort : IDisposable
        {
            /// <summary>
            /// Infinite timeout value to use for GetQueuedCompletedStatus.
            /// </summary>
            private const UInt32 INFINITE_TIMEOUT = unchecked((UInt32) Timeout.Infinite);

            /// <summary>
            /// An invalid file handle value.
            /// </summary>
            private IntPtr INVALID_FILE_HANDLE = unchecked((IntPtr) (-1));

            /// <summary>
            /// An invalid I/O completion port handle value.
            /// </summary>
            private IntPtr INVALID_IOCP_HANDLE = IntPtr.Zero;

            /// <summary>
            /// The I/O completion porth handle.
            /// </summary>
            private readonly SafeFileHandle _handle;

            /// <summary>
            /// Initializes the I/O completion port.
            /// </summary>
            ...
            public IOCompletionPort(Int32 maxConcurrencyLevel)
            {
                ...
                _handle = CreateIoCompletionPort(INVALID_FILE_HANDLE, INVALID_IOCP_HANDLE, UIntPtr.Zero,
                                                  (UInt32) maxConcurrencyLevel);
            }
            ...
            public bool WaitOne()
            {
                ...
                if (
                    !GetQueuedCompletionStatus(_handle.DangerousGetHandle(), out lpNumberOfBytes, out lpCompletionKey,
                                               out lpOverlapped, INFINITE_TIMEOUT))
                {
            ...
            /// <returns>
            /// If the function succeeds, the return value is the handle to an I/O completion port.  If the function fails, the return value is NULL.
            /// </returns>
            [DllImport("kernel32.dll", SetLastError = true)]
            private static extern SafeFileHandle CreateIoCompletionPort(
                IntPtr fileHandle, IntPtr existingCompletionPort, UIntPtr completionKey,
            ...
            private static extern Boolean GetQueuedCompletionStatus(
                IntPtr completionPort, out UInt32 lpNumberOfBytes, out IntPtr lpCompletionKey, out IntPtr lpOverlapped,
            ...
            private static extern Boolean PostQueuedCompletionStatus(
                SafeFileHandle completionPort, IntPtr dwNumberOfBytesTransferred, IntPtr dwCompletionKey,
                IntPtr lpOverlapped);
        }
    }
}