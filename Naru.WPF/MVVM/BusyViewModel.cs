...
namespace Naru.WPF.MVVM
{
    public class BusyViewModel : ViewModel, ISupportBusy
    {
        ...
        public BusyViewModel(ILog log, IScheduler scheduler) 
            : base(log)
        {
        ...
        {
            return Task.Factory.StartNew(() => Active(message), _scheduler.Dispatcher)
                .Select(() => Unit.Default);
        }

        public Task<Unit> InActiveAsync()
        {
            return Task.Factory.StartNew(() => InActive(), _scheduler.Dispatcher)
                .Select(() => Unit.Default);
        }
    }
}